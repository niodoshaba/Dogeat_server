<?php
    ...
    class VegDatabase{
        ...
        function EachPageProductData($page,$per_page){
            $page_count=($page-1)*$per_page;
            $pdo = new DbContext();
            $sort_name = $_SESSION['sort_name'];
            $sort_by = $_SESSION['sort_by'];
            $sql = "select product.pro_no,product.pro_name,product.pro_price,product.pro_status,product.pro_img,catalog.cata_name,product.pro_all_info
            from product, catalog where product.cata_no = catalog.cata_no ORDER BY `product`.`$sort_name` $sort_by
            LIMIT $page_count,$per_page";
            $customApi = $pdo->query($sql);
            if( $customApi->rowCount()==0)
            {
                return false;
        ...
        function insertProductInfo($add_product_catalog,$add_product_name,$add_product_price,$add_product_status,$add_product_img,$add_product_info,$add_product_deadtime,$add_product_element,$add_product_content){
            $pdo = new DbContext();
            $sql = "INSERT INTO `product`(`cata_no`, `pro_name`, `pro_price`, `pro_status`, `pro_img`,`pro_all_info`) 
            VALUES ('$add_product_catalog','$add_product_name','$add_product_price','$add_product_status',
        ...
        function UpdateProductInfo($product_num,$product_catalog,$product_name,$product_price,$product_status,$product_img,$product_info,$product_deadtime,$product_element,$product_content){
            $pdo = new DbContext();
            $sql = "UPDATE `product` SET `cata_no`=$product_catalog,`pro_name`='$product_name',`pro_price`='$product_price',`pro_status`='$product_status',
